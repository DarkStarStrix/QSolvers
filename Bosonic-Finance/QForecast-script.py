...
class BosonicFinance:
    def __init__(self, stock, start_date, end_date):
        ...
        self.rng = np.random.default_rng ()  # Create a default random number generator
    ...
    def measure_quantum_state(self, psi):
        return np.array ([np.abs ((psi.dag () * qt.Qobj (
            [[np.cos (theta / 2), -np.sin (theta / 2)], [np.sin (theta / 2), np.cos (theta / 2)]]).dag () * psi)) ** 2
                          for theta in np.linspace (0, 2 * np.pi, len (self.stock_data))])

    def forecast(self):
        probabilities = self.measure_quantum_state (self.create_quantum_state ())
        return self.rng.choice (self.stock_data, size=8,
                                p=probabilities / np.sum (probabilities))  # Use rng.choice instead of np.random.choice
    ...
    def plot_predicted_stock_price(self):
        ...
        fig = go.Figure (data=[go.Scatter (x=self.data.index [:len (self.stock_data)], y=self.stock_data, mode='lines',
                                           name='Historical Data', line=dict (color='green')),
                               go.Scatter (x=forecast_dates, y=forecasted_data, mode='lines', name='Forecasted Data',
                                           line=dict (color='blue')),
                               go.Scatter (x=forecast_dates, y=actual_data, mode='lines', name='Actual Data',
                                           line=dict (color='orange'))],
                         layout=go.Layout (title='Stock Data Over Time', xaxis=dict (title='Date'),
                                           yaxis=dict (title='Stock Data'), showlegend=True))
        fig.show ()
